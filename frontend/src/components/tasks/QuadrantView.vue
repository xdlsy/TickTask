    ...
    <div class="quadrants-grid">
      <div v-for="q in ([1,2,3,4] as Quadrant[])" :key="q" class="quadrant" @dragover.prevent @drop="onDrop($event, q)">
        ...
        <div class="quadrant-tasks">
          <TaskCard v-for="task in tasksByQuadrant[q]" :key="task.id" :task="task" mode="row" @drag-start="onDragStart" @edit="onEditTask" @complete="onCompleteTask" @delete="onDeleteTask" />
        </div>
      </div>
    </div>
...
function onDragStart(e: DragEvent, t: Task) { draggedTask.value = t; if (e.dataTransfer) e.dataTransfer.effectAllowed = 'move' }
...
function onEditTask(t: Task) { editingTask.value = t; showForm.value = true }
...
<style scoped>
.quadrant-view { padding: 0; height: 100% }
